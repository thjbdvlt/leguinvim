...
impl Line {
    ...
    // with proportional fonts, we want the sign column (i.e. gutter)
    // to ends at the same x-position for all rows. So we need to get it's length.
    // for that, we use two hightlight groups: LineNr and CursorLineNr.
    // see: State.hl_attr_define()
    pub fn sign_column_len(&self) -> usize {
        let mut sign_column_len = 0;
        for (col, cell) in self.line.iter().enumerate() {
            if cell.hl.sign_column {
                sign_column_len = col;
            }
        }
        sign_column_len
