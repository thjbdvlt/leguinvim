...
impl UiModel {
    ...
    pub fn resized(&self, columns: usize, rows: usize) -> Self {
        ...
        // - a new OK grid, above.
        // - an old empty grid, below.
        for row in 0..std::cmp::min(rows, self.rows) {
            let line = &self.model[row];
            for col in 0..std::cmp::min(columns, self.columns) {
                let cell = &line[col];
                new_model.model[row][col] = cell.clone();
            }
        }
        new_model
    }
