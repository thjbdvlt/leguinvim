...
pub use self::redraw_handler::{NvimCommand, RedrawMode};
...
pub fn start<'a>(
    ...
    cmd.arg("--embed")
        ...
        .arg(format!(
            "let &rtp.=',{}'",
            env::var("LEGUINVIM_RUNTIME_PATH").unwrap_or_else(|_| env!("RUNTIME_PATH").into())
        ))
...
pub async fn post_start_init(
    ...
    if !api_info.ext_multigrid {
        return Err(NvimInitError::new_missing_capability("ext_multigrid"));
    }
