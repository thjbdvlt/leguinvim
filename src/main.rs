...
pub struct Args {
    ...
    /// the equivalent LEGUINVIM arguments should be used instead of being passed via this option
    /// whenever possible.
    #[arg(last = true)]
...
fn main() {
    ...
    if std::env::var("LEGUINVIM_CLI_TEST_MODE") == Ok("1".to_string()) {
    ...
    glib::set_program_name(Some("leguinvim"));

    let app = if cfg!(debug_assertions) {
        gtk::Application::new(Some("com.github.thjbdvlt.leguinvimDebug"), app_flags)
    } else {
        gtk::Application::new(Some("com.github.thjbdvlt.leguinvim"), app_flags)
    };
